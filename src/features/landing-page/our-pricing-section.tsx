      ...
      <div className="mt-12 flex flex-wrap items-center justify-center gap-6">
        {data.map((item, index) => (
          <div key={index} className="group w-full max-w-80">
            <div
              className={`flex flex-col items-center justify-center rounded-xl border border-slate-200 p-6 text-center transition-all duration-300 group-hover:-translate-y-1 ${item.mostPopular ? "bg-gray-800 text-white" : "bg-white"}`}
            >
              ...
              <p className="mt-4 text-2xl font-semibold">
                {item.price}{" "}
                <span
                  className={`text-sm font-normal ${item.mostPopular ? "text-gray-400" : "text-gray-500"}`}
                >
                  /month
                </span>
              </p>
              <button
                className={`mt-4 w-full rounded-lg bg-gray-100 py-2.5 font-medium text-gray-800 transition ${item.mostPopular ? "hover:opacity-90" : "hover:bg-gray-200/70"}`}
              >
                {item.buttonText}
              </button>
            </div>
